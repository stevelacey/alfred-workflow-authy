...
class AlfredAuthy(alfred.AlfredWorkflow):
...
"""
    ...
    def __init__(self, config_file="~/.authy", max_results=20):
        ...
        try:
            ...
            item = self.exception_item(
                title="{}: Invalid syntax".format(self._config_file), exception=e
            )
    ...
    def account_item(self, account, token, uid=None):
        return alfred.Item(
            {"uid": alfred.get_uid(uid), "arg": token, "autocomplete": account},
            account,
            token,
            "icon.png",
        )
    ...
    def config_file_is_empty_item(self):
        return self.warning_item(
            title="Authy is not yet configured",
            message="You must add your secrets to "
            "the {} file (see documentation)".format(self._config_file),
        )
...
def main(action, query):
    alfred_authy = AlfredAuthy()
    alfred_authy.route_action(action, query)
